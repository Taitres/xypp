...
import com.taitres.xypp.utils.AlgorithmUtils;
...
//        }).map(this::getSafetyUser).collect(Collectors.toList());
    ...
    public List<User> matchUsers(long num, User loginUser) {
        ...
        // 用户列表的下标 => 相似度
        List<Map<User, Long>> list = new ArrayList<>();
        // 依次计算所有用户和当前用户的相似度
        for (int i = 0; i < userList.size(); i++) {
            ...
            Map<User, Long> mapDistance = new HashMap<>();
            mapDistance.put(user, distance);
            list.add(mapDistance);
        }
        // 按编辑距离由小到大排序
//        List<Map<User, Long>> topUserPairList = list.stream()
//                .sorted((a, b) -> (int) (a.getValue() - b.getValue()))
//                .limit(num)
//                .collect(Collectors.toList());
        List<Map.Entry<User, Long>> topUserPairList = list.stream()
